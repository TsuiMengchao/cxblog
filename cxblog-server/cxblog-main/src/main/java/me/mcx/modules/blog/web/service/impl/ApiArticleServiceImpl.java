...
import me.mcx.modules.blog.mapper.*;
...
public class ApiArticleServiceImpl implements ApiArticleService {
    ...
    public ResponseResult selectArticleList(Integer categoryId,Integer tagId,String orderByDescColumn) {
        ...
        articlePage.getRecords().forEach(item ->{
            ...
            //获取文章
            int collectCount = Math.toIntExact(collectMapper.selectCount(new LambdaQueryWrapper<ArticleCollect>().eq(ArticleCollect::getArticleId, item.getId())));
            item.setCollectCount(collectCount);
            //判断当前登录用户是否收藏该文章 标记为收藏
            if (SecurityUtils.getLoginIdDefaultNull() != null) {
                collectCount = Math.toIntExact(collectMapper.selectCount(new LambdaQueryWrapper<ArticleCollect>().eq(ArticleCollect::getArticleId, item.getId())
                        .eq(ArticleCollect::getUserId, SecurityUtils.getCurrentUserId())));
    ...
    public ResponseResult selectArticleInfo(Integer id) {
        ...
        //获取收藏量
        int collectCount = Math.toIntExact(collectMapper.selectCount(new LambdaQueryWrapper<ArticleCollect>().eq(ArticleCollect::getArticleId, id)));
        ...
        //获取评论数量
        List<ArticleComment> articleComments = commentMapper.selectList(
                new LambdaQueryWrapper<ArticleComment>().eq(ArticleComment::getArticleId, id));
        apiArticleInfoVO.setCommentCount(articleComments.size());
        //获取点赞数量
        ...
        if (userId != null){
            ...
            if(apiArticleInfoVO.getReadType() == ReadTypeEnum.COMMENT.index){
                Integer count = Math.toIntExact(commentMapper.selectCount(new LambdaQueryWrapper<ArticleComment>().eq(ArticleComment::getUserId, userId)));
            ...
            //校验用户是否收藏文章
            int collect = Math.toIntExact(collectMapper.selectCount(new LambdaQueryWrapper<ArticleCollect>().eq(ArticleCollect::getUserId, userId).eq(ArticleCollect::getArticleId, id)));
...
//        List<ArticleSearchVO> articleSearchVOS = searchStrategyContext.executeSearchStrategy(strategy, keywords);
    ...
    private void setCommentAndLike(ApiArticleListVO item) {
        ...
        Integer commentCount = Math.toIntExact(commentMapper.selectCount(new LambdaQueryWrapper<ArticleComment>()
                .eq(ArticleComment::getArticleId, item.getId())));
        //获取点赞数量
        Map<String, Object> map = redisService.getCacheMap(ARTICLE_LIKE_COUNT);
        if (map!= null && map.size() > 0){
