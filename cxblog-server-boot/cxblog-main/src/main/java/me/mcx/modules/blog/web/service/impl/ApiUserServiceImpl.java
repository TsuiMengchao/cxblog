...
public class ApiUserServiceImpl implements ApiUserService {
    ...
    private final String[] userAvatarList = {"http://七牛云.辰雪博客.赵浩悦.我爱你/buxie.png","http://七牛云.辰雪博客.赵浩悦.我爱你/daizhi.png",
            "http://七牛云.辰雪博客.赵浩悦.我爱你/fennu.png","http://七牛云.辰雪博客.赵浩悦.我爱你/jingxi.png","http://七牛云.辰雪博客.赵浩悦.我爱你/kaixin.png",
            "http://七牛云.辰雪博客.赵浩悦.我爱你/shuanshuai.png"};
    ...
    public ResponseResult emailLogin(EmailLoginDTO vo, HttpServletRequest request) throws Exception {
        ...
        UserInfoVO userInfoVO = UserInfoVO.builder()
                .id(Math.toIntExact(jwtUserDto.getUser().getId()))
    ...
    public ResponseResult wxIsLogin(String loginCode, HttpServletRequest request) {
        User user = (User) redisService.getCacheObject(RedisConstants.WX_LOGIN_USER + loginCode);
        if (user == null) {
    ...
    public String wechatLogin(WxMpXmlMessage message) {
        ...
        User user = userMapper.selectOne(new LambdaQueryWrapper<User>().eq(User::getWeixin_id, message.getFromUser()));
        if (Objects.nonNull(user)) {
            ...
            user = User.builder()
                    ...
                    .username("weixin"+message.getFromUser())
                    .nickName(nickname)
                    ...
                    .weixin_id(message.getFromUser())
                    .build();
            userService.create(user);
            user = userService.findByName("weixin"+message.getFromUser());
        }

        //修改redis缓存 以便监听是否已经授权成功
        redisService.setCacheObject(RedisConstants.WX_LOGIN_USER + content, user, 60, TimeUnit.SECONDS);
    ...
    public void authLogin(AuthResponse response, String source, HttpServletResponse httpServletResponse, HttpServletRequest request) throws IOException {
        ...
        String ipSource = me.mcx.utils.StringUtils.getCityInfo(ipAddress);
        // 判断是否已注册
        User user =null;
        switch (source) {
            case "qq":
        ...
        httpServletResponse.sendRedirect(callback_url+"?token=" + finalToken);
    }
    ...
    public ResponseResult getUserCount(String id) {
        ...
        Integer collectCount = Math.toIntExact(collectMapper.selectCount(new LambdaQueryWrapper<ArticleCollect>().eq(ArticleCollect::getUserId, id)));
